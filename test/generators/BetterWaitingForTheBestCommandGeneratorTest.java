...
public class BetterWaitingForTheBestCommandGeneratorTest {
	...
	public void command_is_up_if_there_is_a_go_upstairs_and_current_direction_is_down() {
		...
		assertThat(nextCommand).is(Command.UP);
		assertThatCurrentFloorIsIncremented(0);
		assertThatCurrentDirectionIsSetTo(0, UP);
	}
	...
	public void command_is_down_if_no_waiting_calls_and_no_go_requests_and_is_not_at_middle_floor() {
		...
		assertThatCurrentDirectionIsSetTo(0, DOWN);
	}
	...
	private void assertThatElevatorIsOpened(int cabin) {
		verify(mockStateManager).setOpened(cabin);
		verify(mockStateManager, times(0)).setClosed(cabin);
	}

	private void assertThatElevatorIsClosed(int cabin) {
		verify(mockStateManager, times(0)).setOpened(cabin);
		verify(mockStateManager).setClosed(cabin);
	}

	private void assertThatCurrentFloorIsIncremented(int cabin) {
		verify(mockStateManager).incrementFloor(cabin);
	}

	private void assertThatCurrentFloorIsDecremented(int cabin) {
		verify(mockStateManager).decrementFloor(cabin);
	}

	private void assertThatCurrentDirectionIsSetTo(int cabin, Direction direction) {
		verify(mockStateManager).setCurrentDirection(cabin, direction);
	}

	private void assertThatAllTheCallsAndGosForTheCurrentFloorAreRemoved(int cabin, int currentFloor, Direction direction) {
